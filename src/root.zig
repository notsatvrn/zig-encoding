...
pub const Codepoint = packed struct {
    ...
    pub fn fromUtf8(utf8: []const u8) !struct { Self, usize } {
        ...
        switch (utf8[0]) {
            0b0000_0000...0b0111_1111 => {
                codepoint.inner = @as(u21, utf8[0]);
            },
            0b1100_0000...0b1101_1111 => {
                codepoint.inner = try std.unicode.utf8Decode2(utf8[0..2]);
                length = 2;
            },
            0b1110_0000...0b1110_1111 => {
                codepoint.inner = try std.unicode.utf8Decode3(utf8[0..3]);
                length = 3;
            },
            0b1111_0000...0b1111_0111 => {
                codepoint.inner = try std.unicode.utf8Decode4(utf8[0..4]);
                length = 4;
            },
    ...
    pub fn fromMutf8(mutf8: []const u8) !struct { Self, usize } {
        var codepoint = Self{};
        var length: usize = 1;

        if (mutf8 == 0) {
            return error.NullByte;
        } else if (mutf8[0] < 0x80) {
            codepoint.inner = @as(u21, mutf8[0]);
        } else if (mutf8.len >= 2 and (mutf8[1] & 0xC0) == 0x80) {
            if ((mutf8[0] & 0xE0) == 0xC0) {
                codepoint.inner = (@as(u21, mutf8[0] & 0b11111) << 6) | @as(u21, mutf8[1] & 0b111111);
                length = 2;
            } else if (mutf8.len >= 3 and (mutf8[0] & 0xF0) == 0xE0 and (mutf8[2] & 0xC0) == 0x80) {
                if (mutf8.len >= 6 and
                    mutf8[0] == 0b11101101 and
                    (mutf8[1] & 0xF0) == 0xA0 and
                    mutf8[3] == 0b11101101 and
                    (mutf8[4] & 0xF0) == 0xB0 and
                    (mutf8[5] & 0xC0) == 0x80)
                {
                    codepoint.inner = (@as(u21, mutf8[1] & 0b1111) << 16) |
                        (@as(u21, mutf8[2] & 0b111111) << 10) |
                        (@as(u21, mutf8[4] & 0b1111) << 6) |
                        @as(u21, mutf8[5] & 0b111111);
                    length = 6;
                } else {
                    codepoint.inner = (@as(u21, mutf8[0] & 0b1111) << 12) | (@as(u21, mutf8[1] & 0b111111) << 6) | @as(u21, mutf8[2] & 0b111111);
                    length = 3;
                }
            } else {
...
inline fn codepointsFromUtf8Alloc(allocator: Allocator, utf8: []const u8) !std.ArrayList(Codepoint) {
    var codepoints = try std.ArrayList(Codepoint).initCapacity(allocator, utf8.len);
    errdefer codepoints.deinit();

    var i: usize = 0;
    var len: usize = 0;
    while (i < utf8.len) : (len += 1) {
...
inline fn codepointsFromUtf16leAlloc(allocator: Allocator, utf16le: []const u16) !std.ArrayList(Codepoint) {
    var codepoints = try std.ArrayList(Codepoint).initCapacity(allocator, utf16le.len);
    errdefer codepoints.deinit();

    var i: usize = 0;
    var len: usize = 0;
    while (i < utf16le.len) : (len += 1) {
...
pub const MutableString = struct {
    ...
    pub fn fromUtf8(allocator: Allocator, utf8: []const u8) !Self {
        return .{
            .codepoints = try codepointsFromUtf8Alloc(allocator, utf8),
        };
    }
    ...
    pub fn fromUtf16le(allocator: Allocator, utf16le: []const u16) !Self {
        return .{
            .codepoints = try codepointsFromUtf16leAlloc(allocator, utf16le),
        };
    }
...
pub const StaticString = struct {
    ...
    pub inline fn fromUtf8Comptime(comptime utf8: []const u8) Self {
        comptime var codepoints: [utf8.len]Codepoint = undefined;

        comptime var i: usize = 0;
        comptime var len: usize = 0;
        inline while (i < utf8.len) : (len += 1) {
            const codepoint = comptime Codepoint.fromUtf8(utf8[i..]) catch @compileError("invalid UTF-8 in string");
    ...
    pub fn fromUtf8Alloc(allocator: Allocator, utf8: []const u8) !Self {
        return .{
            .codepoints = (try codepointsFromUtf8Alloc(allocator, utf8)).items,
        };
    }
    ...
    pub fn fromUtf16leAlloc(allocator: Allocator, utf16le: []const u16) !Self {
        return .{
            .codepoints = (try codepointsFromUtf16leAlloc(allocator, utf16le)).items,
        };
    }
